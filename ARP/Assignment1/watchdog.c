...
pid_t server_pid, window_pid, keyboard_pid, drone_pid, wd_pid;
...
// siginfo and server_pid is not the same???

int main(int argc, char* argv[]){
    initscr();
    // pid_t all_pids[5];
    // sscanf(argv[1],"%d %d %d %d %d", &all_pids[0], &all_pids[1], &all_pids[2], &all_pids[3], &all_pids[4]);
    sscanf(argv[1],"%d %d|%d %d|%d %d|%d %d", &wd_server[0], &wd_server[1], 
    ...
    close(wd_keyboard[0]);
    close(wd_window[1]);
    printf("%d %d|%d %d|%d %d|%d %d", wd_server[0], wd_server[1], 
                                                wd_window[0],wd_window[1],
                                                key_wd[0],key_wd[1],
                                                wd_drone[0],wd_drone[1]);
    wd_pid=getpid();
    ...
    while(1){
        ...
        // if(server_timer > TIMER_THRESH || window_timer > TIMER_THRESH || 
        //     drone_timer > TIMER_THRESH || keyboard_timer > TIMER_THRESH){
        if(server_timer > TIMER_THRESH || window_timer > TIMER_THRESH || drone_timer > TIMER_THRESH ){
