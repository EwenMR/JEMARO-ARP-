...
def find_token(marker_type):
    ...
    dist = 100
    for token in R.see():
        if token.dist < dist and token.info.marker_type == marker_type:
            dist = token.dist
            rot_y = token.rot_y
    if dist == 100:
        return -1, -1
...
def silver_token():
    while 1:
        dist, rot_y = find_token('silver-token')  # we look for markers
        if dist==-1:
...
def gold_token():
    while 1:
        dist, rot_y = find_token('gold-token')  # we look for markers
        if dist==-1:
            print("I don't see any token!!")
            exit()  # if no markers are detected, the program ends
        elif dist <d_th: 
